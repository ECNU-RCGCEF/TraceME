    ...
    foreach ($formItems as $field=>$items){
		...
		if($input_type){
			...
			foreach($items as $i=>$item){				
				...
				if (strtolower($field) == "variablename"){
				    echo '<input type="'.$input_type.'" name="'.lcfirst($field).'" value="'.$item.'" id="'.strtolower($field).'_'.$i.'" checked=true/>'.$item;
				    echo '</label>';
				}
				else{
