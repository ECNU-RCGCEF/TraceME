		...
		if($data['status'] == 'finished'){
			...
			foreach ($data['resultFile'] as $file){
				switch ($file['type']){
					...
					case 'nc':
						$ncs[] = $file['url'];
                                                $filenames_nc[] = end(explode('/',$file['filename']));
						break;
					case 'txt':
						$txts[] = $file['url'];
						break;
			...
			if($ncs || $txts){
				...
				foreach ($ncs as $url){
                                       ...
                                       // $file_name1 = pathinfo($path_parts['path']);
                                       // $file_name  = $file_name1['basename'];
                                        $file_name = $filenames_nc[$i];
                                        $file_name1 = mb_substr($file_name, 0,10, 'UTF-8') . str_repeat('.', 3).mb_substr($file_name, -1, 10, 'UTF-8'); 
				        echo '<a title="'.$filenames_nc[$i].'" href="'.$url.'" target="_blank">Nc Result Download</a>';	
                                        $i++;
				}
				foreach ($txts as $url){
					$i++;
